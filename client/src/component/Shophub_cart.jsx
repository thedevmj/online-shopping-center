...
export default function Shophub_cart({ category, filter, search, setsearch }) {
  ...
  const handlefavoritebook = useCallback(async (book) => {
    if (isAdmin) {
      showToast("Admins cannot use favorites.", "info");
      return;
    }
 ...
 const filteredBooks = useMemo(() => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 p-4 sm:p-6 relative overflow-hidden">
      ...
      <div className="relative z-10 mx-auto max-w-7xl">
        <section className="relative overflow-hidden rounded-4xl border border-lime-400/20 bg-slate-950/50 shadow-[0_20px_60px_rgba(163,230,53,0.18)] backdrop-blur-xl">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(163,230,53,0.26),transparent_35%),radial-gradient(circle_at_bottom_right,rgba(59,130,246,0.22),transparent_35%)]" />
          <div className="relative grid gap-10 px-6 py-8 sm:px-10 lg:grid-cols-[1.2fr_0.8fr] lg:px-16 lg:py-12">
            <div className="flex flex-col justify-center">
              <span className="mb-4 inline-flex w-fit items-center gap-2 rounded-full border border-lime-400/30 bg-lime-500/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-lime-300">
                New Season Drop
              </span>
              ...
              <div className="mt-8 flex flex-col gap-4 sm:flex-row">
                ...
                <button
                  ...
                  className="inline-flex items-center justify-center rounded-full border border-white/20 bg-white/5 px-6 py-3 text-sm font-bold text-white transition hover:border-lime-400/60 hover:bg-lime-500/10"
                >
                  Save Featured Pick
                </button>
              </div>
              ...
              <div className="mt-8 flex flex-wrap gap-4 text-sm text-slate-200">
                <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
                  <p className="text-2xl font-black text-lime-300">20k+</p>
                  <p className="text-slate-300">Readers</p>
                </div>
                <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
                  <p className="text-2xl font-black text-lime-300">1.4k</p>
                  <p className="text-slate-300">Book Picks</p>
                </div>
                <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
                  <p className="text-2xl font-black text-lime-300">4.9/5</p>
            ...
            <div className="relative flex items-center justify-center">
              <div className="absolute -inset-4 rounded-4xl bg-lime-500/10 blur-2xl" />
                ...
                {featuredBook ? (
                  <>
                    ...
                    <div className="mt-5 flex items-start justify-between gap-4">
                      ...
                      <span className="rounded-full border border-lime-400/30 bg-lime-500/10 px-3 py-1 text-base font-bold text-lime-300">
                    ...
                    <div className="mt-5 flex items-center justify-between gap-3">
                      <button
                        ...
                        className="flex-1 rounded-full bg-linear-to-r from-lime-400 to-lime-500 px-4 py-3 text-sm font-bold text-slate-950 shadow-lg shadow-lime-500/30"
                      >
                        Buy Now
                      </button>
                      <button
                        onClick={() => handlefavoritebook(featuredBook)}
                        className="rounded-full border border-white/15 bg-white/5 p-3 text-white transition hover:border-lime-400/50 hover:text-lime-300"
                ...
                <div className="backdrop-blur-xl bg-white/10 rounded-3xl p-12 text-center border border-white/20">
                  ...
                  {search && search.trim() && (
                    <button
                      ...
                      className="mt-4 px-6 py-2 bg-lime-500/20 text-lime-300 rounded-xl hover:bg-lime-500/30 transition-colors border border-lime-500/30"
                    >
                      Clear search
                    </button>
                  )}
              ...
              filteredBooks.map((book) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      className="bg-linear-to-r from-lime-400 to-lime-500 hover:from-lime-500 hover:to-lime-600 text-slate-950 px-6 py-3 rounded-2xl font-semibold transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105 backdrop-blur-sm border border-white/20"
